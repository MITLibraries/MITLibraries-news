 ...
 *
 * This appears to have been a test template from early in the development
 * of the theme. It can probably be deleted, but I am still verifying this.
 * - Matt Bernhardt, May 2016
 *
 * @package WordPress
...
<?php
/**
 * The top section of the page displays featured / sticky posts.
 */

// Part A defines the query for this loop.
...
<?php
/**
 * The bottom section of the page displays everything else.
 */
// Part A defines the query for this loop.
$args = array(
	'posts_per_page'      => 9,
...
<?php
		...
		if ( $post->post_type == 'spotlights' ) {
?>
						<div class="featuredColImg">
							<img src="/wp-content/themes/mit-libraries-news/images/info.png" alt="featured" width="31" height="27" />
						</div>
<?php
		}
		if ( get_field( 'mark_as_new' ) === true ) {
?>
...
?>
						<div class="event"><?php echo $date->format( 'F, j Y' ); ?>&nbsp;&nbsp; &nbsp;
							<span class="time">
<?php
				if ( get_field( 'event_start_time' ) ) {
					echo the_field( 'event_start_time' );
...
<?php
			// This is the end of the final else condition.
		}
?>
...
<?php
		if ( get_post_type( get_the_ID() ) == 'bibliotech' ) {
			...
			$rCat = count( $category );
			$r = rand( 0, $rCat -1 );
			echo '<a title="'.$category[ $r ]->cat_name.'" href="'.get_category_link( $category[ $r ]->term_id ).'">'.$category[ $r ]->cat_name.'</a>';
		}
?>
						<span class="mitDate">
